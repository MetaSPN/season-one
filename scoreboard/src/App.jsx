...
const GITHUB_REPO = import.meta.env.VITE_GITHUB_REPO || "leoguinan/stakes-season-one";
...
function getDayNumber() {
  const now = new Date();
  return Math.max(0, Math.floor((now - SEASON_START) / 86400000));
}

function getSeasonProgress() {
  const total = 90;
  return Math.min(100, (getDayNumber() / total) * 100);
}
...
function TowelBar({ level, max = 5, color }) {
  ...
  return (
    <div style={{ display: "flex", gap: 2, alignItems: "center" }}>
      ...
      <span style={{ marginLeft: 6, fontFamily: "'IBM Plex Mono', monospace", fontSize: 11, color: level > 0 ? color : "#4a5568", opacity: 0.9 }}>
        {level}
      </span>
    </div>
  );
...
function PairCard({ pair, index, expanded, onToggle }) {
  ...
  const reportLink = reportUrl(pair.reportPath);
  return (
    <div
      style={{
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        <div style={{ flex: 1 }}>
          ...
          <div style={{ display: "flex", alignItems: "baseline", gap: 6 }}>
            <span style={{ fontFamily: "'Newsreader', serif", fontSize: 18, fontWeight: 600, color: "#e2e8f0" }}>
              {pair.creator.name}
            </span>
            <span style={{ color: "#4a5568", fontSize: 14 }}>→</span>
            <span style={{ fontFamily: "'Newsreader', serif", fontSize: 18, fontWeight: 600, color: "#93c5fd" }}>
              {pair.agent.name}
            </span>
          </div>
          <div style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 10, color: "#6b7280", letterSpacing: 0.5, marginTop: 2 }}>
            {pair.archetype}
          </div>
        </div>
        ...
        <div style={{ display: "flex", gap: 20, alignItems: "center" }}>
          ...
          <div style={{ textAlign: "center", minWidth: 48 }}>
            ...
            <div style={{
              ...
              fontSize: 10,
              fontWeight: 600,
              color: CS_COLORS[pair.controlState],
              letterSpacing: 1,
            }}>
              {pair.controlState}
            </div>
...
export default function Scoreboard() {
  ...
  return (
    <div style={{
      ...
      <div style={{ position: "relative", zIndex: 2, maxWidth: 880, margin: "0 auto", padding: "32px 24px" }}>
        ...
        <div style={{ marginBottom: 28 }}>
          ...
          <p style={{
            ...
            Tracking the validation distance between creators and their AI agents.
            <br />
            Seven pairs. Measured publicly. Audited honestly.
          </p>
        </div>
        ...
        <div style={{
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
            <div style={{ display: "flex", alignItems: "baseline", gap: 12 }}>
              ...
              <span style={{ fontFamily: "'Newsreader', serif", fontSize: 22, fontWeight: 700, color: "#e2e8f0" }}>
                Day {dayNum}
              </span>
            ...
            {nextAudit && (
              <div style={{ textAlign: "right" }}>
                ...
                <span style={{ fontSize: 10, color: nextAudit.major ? "#818cf8" : "#6b7280", fontWeight: nextAudit.major ? 600 : 400 }}>
                  {nextAudit.label}
                </span>
              </div>
            )}
          ...
          <div style={{ position: "relative", height: 6, background: "rgba(255,255,255,0.04)", borderRadius: 3 }}>
            ...
            {AUDIT_DATES.map((a) => {
              const auditDay = Math.floor((new Date(a.date) - SEASON_START) / 86400000);
              const pct = (auditDay / 90) * 100;
              return (
                <div key={a.date} style={{
                  ...
                  width: a.major ? 3 : 1,
                  height: 12,
                  background: a.major ? "#818cf8" : "rgba(255,255,255,0.2)",
                  borderRadius: 1,
                }} />
              );
            })}
          ...
          <div style={{ display: "flex", justifyContent: "space-between", marginTop: 6 }}>
            {AUDIT_DATES.map((a) => (
              <span key={a.date} style={{
                fontSize: 8,
                color: a.major ? "#818cf8" : "#4a5568",
                letterSpacing: 0.5,
                fontWeight: a.major ? 600 : 400,
              }}>
                {a.label}
              </span>
            ))}
        ...
        <div style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            ...
            {Object.entries(CS_COLORS).map(([k, v]) => (
              <span key={k} style={{ fontSize: 9, color: v, marginRight: 4 }}>{k}</span>
            ))}
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            <span style={{ fontSize: 9, color: "#4a5568", letterSpacing: 1 }}>Val. Dist.: </span>
            {Object.entries(VD_COLORS).map(([k, v]) => (
              <span key={k} style={{ fontSize: 9, color: v, marginRight: 4 }}>{k}</span>
            ))}
          </div>
        </div>
        ...
        {/* Pair cards */}
        {loading && (
          <div style={{ padding: 48, textAlign: "center", color: "#6b7280", fontSize: 12 }}>
            Loading...
          </div>
        )}
        {error && (
          <div style={{ padding: 48, textAlign: "center", color: "#ef4444", fontSize: 12 }}>
            {error}
          </div>
        )}
        {!loading && !error && (
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            {pairs.map((pair, i) => (
              <PairCard key={pair.id} pair={pair} index={i} expanded={expanded} onToggle={handleToggle} />
            ))}
          </div>
        )}
        ...
        <div style={{
          ...
          <p style={{
            ...
            Every pair is a thesis. Every $TOWEL stake is a commitment.
            <br />
            What do you believe? Who's aligned? Who's drifting?
          </p>
          <div style={{
            display: "flex",
            ...
            <a
              href={`https://github.com/${GITHUB_REPO}/tree/main/players`}
              target="_blank"
              rel="noopener noreferrer"
              style={{
                padding: "8px 20px",
        ...
        <div style={{
          ...
          <div style={{ fontSize: 9, color: "#4a5568", letterSpacing: 1 }}>
            Marvin · MBH Conviction Signal Engine v0.1 · MetaSPN Season 1
          </div>
          <div style={{
            fontFamily: "'Newsreader', serif",
            fontSize: 11,
      ...
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Newsreader:ital,wght@0,400;0,500;0,600;0,700;1,400&family=IBM+Plex+Mono:wght@400;500;600&display=swap');
